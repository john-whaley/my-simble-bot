...
- 如何把项目上传到 GitHub
- 如何在 Linux，尤其是 Debian 13 VPS 上部署
- 如何使用 Docker Compose 启动、更新、查看日志

## 1. 上传 GitHub 前需要确认的文件
...
本项目已经通过 `.gitignore` 处理了这些内容。

## 2. 本地初始化 Git 并上传到 GitHub

如果项目还没有 Git 仓库，可以在项目根目录执行：

```bash
git init
git add .
git commit -m "init my-simple-bot"
```
